...
#' @examples
#' small_matrix <- array(1:625, c(5, 5, 5))
#' tubeMedian(small_matrix)
#' @export
tubeMedian <- function(x, na.rm = FALSE, mis_val = -2147483648) {
  if(is.integer(x)) {
    cpp_tubemedian_int(x, na_rm = na.rm, mis_val = mis_val)
  } else {
    cpp_tubemedian_num(x, na_rm = na.rm)
  }
}
